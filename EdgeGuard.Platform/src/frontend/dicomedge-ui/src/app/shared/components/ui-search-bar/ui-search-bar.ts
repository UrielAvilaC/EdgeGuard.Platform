...
@Component({
  ...
  template: `
    <mat-form-field appearance="outline" class="w-full" subscriptSizing="dynamic">
      <fa-icon matPrefix [icon]="searchIcon" class="mr-2 text-gray-400" />
      <input
        matInput
      ...
      />
      @if (value()) {
        <button matSuffix type="button" class="text-gray-400 hover:text-gray-600" (click)="clear()" aria-label="Clear search">
  ...
  styles: `
    :host { display: block; }
    mat-form-field {
      --mdc-outlined-text-field-container-shape: var(--eg-radius-md);
      --mdc-outlined-text-field-outline-color: var(--eg-border);
    }
  `,
})
