...
/// <summary>
/// Single source of truth for the Hub's default <c>system_settings</c> rows.
/// Referenced by both the startup seeder (<c>HubSettingsSeed</c>, Persistence) and the
/// manual re-seed path (<c>SystemSettingsService.SeedDefaultsAsync</c>, Application) so the
/// two never diverge.
/// </summary>
public static class HubSettingsDefaults
{
    /// <summary>Builds the full canonical set of default settings.</summary>
    public static IReadOnlyList<SystemSetting> Build() =>
    [
        // ── HL7 ─────────────────────────────────────────────────────────────
        ...
        // ── WhatsApp (channel-specific provider config) ──────────────────────
        ...
        EncRow(HubSettingKeys.WhatsApp.ProviderConfig,
               """{"AccountSid":"REPLACE_WITH_TWILIO_ACCOUNT_SID","AuthToken":"REPLACE_WITH_TWILIO_AUTH_TOKEN","PhoneNumber":"+REPLACE_WITH_APPROVED_WA_NUMBER","MessagingServiceSid":"REPLACE_WITH_MESSAGING_SERVICE_SID"}""",
               Cat.WhatsApp, "Provider Configuration (JSON)", VT.String),
        ...
        Row(HubSettingKeys.DataRetention.NotificationDays,           "60",     Cat.Retention, "Notification Outbox Retention (days)", VT.Int),
        Row(HubSettingKeys.DataRetention.NodeOutboxDays,             "14",     Cat.Retention, "Node Outbox Retention (days)",         VT.Int),
