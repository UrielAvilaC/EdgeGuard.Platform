...
public sealed class MessageDispatchHostedService : BackgroundService
{
        ...
        foreach (var message in batch)
        {
            try
            {
                ...
                var request = new NodeDispatchRequest
                {
                    ...
                    // MWL-FIX-3: forward MWL-relevant fields end-to-end so the
                    // Node can filter and respond properly to C-FIND queries.
                    PatientBirthDate         = message.PatientBirthDate,
                    PatientSex               = message.PatientSex,
                    ScheduledDateTime        = ParseHl7StudyDate(message.StudyDate),
                    ReferringPhysicianName   = null,   // not extracted from HL7 yet
                    StudyInstanceUid         = null,   // generated by Node on receive
                    ScheduledStationAeTitle  = null,   // OBR-21/22 — populate when parser supports it
                    ScheduledPerformingPhysicianName = null,
                    ScheduledProcedureStepId         = message.ProcedureId,
                };
    ...
    /// <summary>
    /// MWL-FIX-3: Parses an HL7 OBR-7 study date string (<c>yyyyMMddHHmmss</c> or
    /// <c>yyyyMMdd</c>) into a UTC <see cref="DateTime"/>. Returns <c>null</c> on
    /// invalid or empty input.
    /// </summary>
    private static DateTime? ParseHl7StudyDate(string? hl7Date)
    {
        if (string.IsNullOrWhiteSpace(hl7Date)) return null;

        if (hl7Date.Length >= 14 && DateTime.TryParseExact(
                hl7Date[..14], "yyyyMMddHHmmss",
                System.Globalization.CultureInfo.InvariantCulture,
                System.Globalization.DateTimeStyles.None,
                out var dt14))
            return DateTime.SpecifyKind(dt14, DateTimeKind.Utc);
