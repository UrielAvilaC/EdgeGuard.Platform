...
public sealed class DiagnosticsOptionsValidator : IValidateOptions<DiagnosticsOptions>
{
    public ValidateOptionsResult Validate(string? name, DiagnosticsOptions options)
    {
        ...
        if (string.IsNullOrWhiteSpace(options.NodeId))
            failures.Add($"{nameof(options.NodeId)} is required.");
        ...
        if (options.HealthChecks.PacsTimeoutSeconds <= 0)
            failures.Add($"{nameof(options.HealthChecks)}.{nameof(options.HealthChecks.PacsTimeoutSeconds)} must be greater than 0.");
